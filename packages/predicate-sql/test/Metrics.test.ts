...
const isCounter = (s: Metric.Metric.Snapshot): s is CounterSnapshot =>
  s.type === "Counter" && s.id === "qadi_predicate_sql_compiled_total";

describe("qadi_predicate_sql_compiled_total", () => {
  it.effect("counts a successful compile, tagged 'compiled'", () =>
    Effect.gen(function* () {
      ...
      const snapshots = yield* isolatedMetrics(
        Effect.gen(function* () {
          yield* compileSql(eq, { dialect: "postgres" });
      ...
      assert.strictEqual(counter?.description, "Predicates compiled by @qadi/predicate-sql's compileSql, tagged by outcome.");
  ...
  it.effect("counts a refusal, tagged 'refused' — a distinct entry from 'compiled'", () =>
    Effect.gen(function* () {
      ...
      const snapshots = yield* isolatedMetrics(
        Effect.gen(function* () {
          yield* Effect.result(compileSql(unsafe, { dialect: "postgres" }));
          return yield* Metric.snapshot;
        }),
      );
