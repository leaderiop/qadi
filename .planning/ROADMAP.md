...
- [x] 01-03-PLAN.md — Configure the permanent nine-package `fixed` group and run the version bump to a shared 0.3.0 [REL-03]
...
- [x] 01-05-PLAN.md — Close the TOCTOU race in `@qadi/react`'s suspense promise that fails the `check (20.19.0)` floor leg, and prove both legs green [COMPAT-01]

**Wave 5** *(gap closure — UAT gap G-01-2)*

- [x] 01-06-PLAN.md — Scope merge gate 22 to the runtimes `apps/website`'s Astro toolchain supports, record the decision as ADR-QD-059, and prove `check (20.19.0)` green end to end [COMPAT-01, REL-03, REL-04]
...
|-------|----------------|--------|-----------|
| 1. Release Readiness & Runtime Verification | 6/6 | In Progress|  |
