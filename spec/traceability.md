...
| [INV-QD-006](invariants.md#inv-qd-006-failure-is-not-denial) | Failure is not denial | Effect error channel; `Evaluate.ts`'s five port calls `Effect.catchCause` a defect into that port's own typed error (BEH-QD-261) | `Evaluate.test.ts`, `Policies.test.tsx`, `port-calls.feature` |
...
| `packages/react/test/settled.test.ts` | `settled` in isolation, exercised through a registry directly (G-01-1, COMPAT-01) |
...
| `packages/react/test/Hydration.test.ts` | BEH-QD-145–152, BEH-QD-259, INV-QD-022, INV-QD-028, INV-QD-045, ADR-QD-041 |
