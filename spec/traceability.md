...
| [ADR-QD-056](decisions/056-audit-companion-package.md) | `@qadi/audit`: a companion package narrows ADR-QD-016 | — (correctness rests on `packages/audit/test/`'s property and scripted-boundary tests; no `INV-QD-NNN` recorded yet, per the ADR's own negative consequence) |
