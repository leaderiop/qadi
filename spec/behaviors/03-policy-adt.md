...
Sixteen variants, discriminated on `_tag`. The type is hand-written first —
...
| `Rules` | An ordered rule table; exactly one rule decides |
...
| `HasCustom` | A named, externally-registered predicate returns `true` |
| `HasSignature` | The subject has an on-file signature matching a meaning |
...
```ts
export type Policy = /* the sixteen-variant union above, hand-written first */;
export const Policy: Schema.Codec<Policy, PolicyEncoded>; // built and type-asserted against it
```
...
export const rules: (rules: ReadonlyArray<Rule>, options?: { readonly combining?: Combining }) => Policy;
